    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑医生挂号</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑医生挂号</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">医生挂号信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6 aaaaaa yisheng">
                                        <label>医生</label>
                                        <div>
                                            <select style="width: 450px" id="yishengSelect" name="yishengSelect"
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 yishengOrderUuidNumberDiv">
                                        <label>门诊号</label>
                                        <input style="width: 450px" id="yishengOrderUuidNumber" name="yishengOrderUuidNumber" class="form-control"
                                               placeholder="门诊号">
                                    </div>
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 guahaoTimeDiv">
                                        <label>挂号日期</label>
                                        <input style="width: 450px" id="guahaoTime-input" name="guahaoTime" type="text" class="form-control layui-input">
                                    </div>
                                    ...
                                    <div class="form-group col-md-6 yishengOrderTypesDiv">
                                        <label>挂号状态</label>
                                        <select style="width: 450px" id="yishengOrderTypesSelect" name="yishengOrderTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            let data = {};
            getContent();
            if(window.sessionStorage.getItem('role') != '医生'){//当前登录用户不为这个
                if($("#yishengId") !=null){
            ...
            httpJson("yishengOrder/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addyishengOrder');
    ...
    // 查询列表
        ...
        function yishengOrderTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=yisheng_order_types", "GET", {}, (res) => {
                if(res.code == 0){
                    yishengOrderTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationYishengordertypesSelect(){
            var yishengOrderTypesSelect = document.getElementById('yishengOrderTypesSelect');
            if(yishengOrderTypesSelect != null && yishengOrderTypesOptions != null  && yishengOrderTypesOptions.length > 0 ){
                for (var i = 0; i < yishengOrderTypesOptions.length; i++) {
                    yishengOrderTypesSelect.add(new Option(yishengOrderTypesOptions[i].indexName,yishengOrderTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(yishengOrderTypesSelect != null && yishengOrderTypesOptions != null  && yishengOrderTypesOptions.length > 0 ) {
            for (var i = 0; i < yishengOrderTypesOptions.length; i++) {
                if (yishengOrderTypesOptions[i].codeIndex == ruleForm.yishengOrderTypes) {//下拉框value对比,如果一致就赋值汉字
                        yishengOrderTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yishengOrderUuidNumber: "required",
                yishengId: "required",
                yonghuId: "required",
                guahaoTime: "required",
                shijianduanTypes: "required",
                yishengOrderTypes: "required",
                beizhuContent: "required",
            },
            messages: {
                yishengOrderUuidNumber: "门诊号不能为空",
                yishengId: "医生不能为空",
                yonghuId: "用户不能为空",
                guahaoTime: "挂号日期不能为空",
                shijianduanTypes: "时间段不能为空",
                yishengOrderTypes: "挂号状态不能为空",
    ...
    // 获取当前详情
    function getDetails() {
        var addyishengOrder = window.sessionStorage.getItem("addyishengOrder");
        if (addyishengOrder != null && addyishengOrder != "" && addyishengOrder != "null") {
            //注册表单验证
            $(validform());
            $("#yishengOrderUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("yishengOrder/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#yishengOrderUuidNumber").val(ruleForm.yishengOrderUuidNumber);
    ...
    function yishengDataBind(){
        ...
        $("#yishengUuidNumber").val(ruleForm.yishengUuidNumber);
        $("#yishengName").val(ruleForm.yishengName);
        $("#yishengValue").val(ruleForm.yishengValue);
        $("#zhiweiValue").val(ruleForm.zhiweiValue);
        $("#yishengShanchang").val(ruleForm.yishengShanchang);
        $("#yishengContent").val(ruleForm.yishengContent);
        $("#shangxiaValue").val(ruleForm.shangxiaValue);
        $("#yishengClicknum").val(ruleForm.yishengClicknum);
    }
    ...
    function yonghuDataBind(){
        ...
        $("#shiyongValue").val(ruleForm.shiyongValue);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationYishengordertypesSelect();
            <!--  初始化级联表的下拉框  -->
