    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑医生收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">医生收藏管理</li>
                        <li class="breadcrumb-item active">编辑医生收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">医生收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 yishengCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="yishengCollectionTypesSelect" name="yishengCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    var tableName = "yishengCollection";
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var yishengCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var yishengOptions = [];
    var yonghuOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("yishengCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addyishengCollection');
    ...
    // 查询列表
        <!-- 查询当前表的所有列表 -->
        function yishengCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=yisheng_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    yishengCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationYishengcollectiontypesSelect(){
            var yishengCollectionTypesSelect = document.getElementById('yishengCollectionTypesSelect');
            if(yishengCollectionTypesSelect != null && yishengCollectionTypesOptions != null  && yishengCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < yishengCollectionTypesOptions.length; i++) {
                    yishengCollectionTypesSelect.add(new Option(yishengCollectionTypesOptions[i].indexName,yishengCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(yishengCollectionTypesSelect != null && yishengCollectionTypesOptions != null  && yishengCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < yishengCollectionTypesOptions.length; i++) {
                if (yishengCollectionTypesOptions[i].codeIndex == ruleForm.yishengCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        yishengCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yishengId: "required",
                yonghuId: "required",
                yishengCollectionTypes: "required",
            },
            messages: {
                yishengId: "医生不能为空",
                yonghuId: "用户不能为空",
                yishengCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addyishengCollection = window.sessionStorage.getItem("addyishengCollection");
        if (addyishengCollection != null && addyishengCollection != "" && addyishengCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("yishengCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationYishengcollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
