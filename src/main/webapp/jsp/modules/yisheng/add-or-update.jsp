                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 yishengTypesDiv">
                                        <label>科室</label>
                                        <select style="width: 450px" id="yishengTypesSelect" name="yishengTypes" class="form-control">
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-md-6 yishengShanchangDiv">
                                        <label>擅长</label>
                                        <input style="width: 450px" id="yishengShanchang" name="yishengShanchang" class="form-control"
                                               placeholder="擅长">
                                    </div>
                                    <div class="form-group  col-md-6 yishengContentDiv">
                                        <label>医生介绍</label>
                                        <input id="yishengContentupload" name="file" type="file">
...
<script>
    ...
    function upload() {
        ...
        $('#yishengContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('yishengContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationYishengtypesSelect(){
            var yishengTypesSelect = document.getElementById('yishengTypesSelect');
            if(yishengTypesSelect != null && yishengTypesOptions != null  && yishengTypesOptions.length > 0 ){
                for (var i = 0; i < yishengTypesOptions.length; i++) {
                    yishengTypesSelect.add(new Option(yishengTypesOptions[i].indexName,yishengTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(yishengTypesSelect != null && yishengTypesOptions != null  && yishengTypesOptions.length > 0 ) {
            for (var i = 0; i < yishengTypesOptions.length; i++) {
                if (yishengTypesOptions[i].codeIndex == ruleForm.yishengTypes) {//下拉框value对比,如果一致就赋值汉字
                        yishengTypesSelect.options[i].selected = true;
                }
            }
        }
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.yishengContent != null && ruleForm.yishengContent != 'null' && ruleForm.yishengContent != '' && $("#yishengContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#yishengContentupload").length>0) {
            var yishengContentEditor = UE.getEditor('yishengContentEditor');
            if (yishengContentEditor.hasContents()) {
                $('#yishengContent-input').attr('value', yishengContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function yishengClicknumChickValue(e){
        ...
        function yishengDeleteChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[1-9][0-9]*$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yishengUuidNumber: "required",
                yishengName: "required",
                yishengPhoto: "required",
                yishengTypes: "required",
                zhiweiTypes: "required",
                yishengShanchang: "required",
                yishengContent: "required",
                shangxiaTypes: "required",
                yishengClicknum: "required",
            },
            messages: {
                yishengUuidNumber: "医生编号不能为空",
                yishengName: "医生名称不能为空",
                yishengPhoto: "医生照片不能为空",
                yishengTypes: "科室不能为空",
                zhiweiTypes: "职位不能为空",
                yishengShanchang: "擅长不能为空",
                yishengContent: "医生介绍不能为空",
                shangxiaTypes: "是否上架不能为空",
                yishengClicknum: "点击次数不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addyisheng != null && addyisheng != "" && addyisheng != "null") {
            //注册表单验证
            $(validform());
            $("#yishengUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationYishengtypesSelect();
