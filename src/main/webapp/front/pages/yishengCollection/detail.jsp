...
<body>
        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.yishengCollectionName}}</cite></a>
                </span>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.yishengCollectionPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.yishengCollectionTypes">类型：
                                {{detail.yishengCollectionValue}}
                            </li>
                        </ul>
        ...
        <div class="col-lg-4" v-if="yishengCollectionRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in yishengCollectionRecommendList" v-bind:key="index" @click="jump('../yishengCollection/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.yishengCollectionPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.yishengCollectionName}}</a>
                                <h6 v-if="item.yishengCollectionNewMoney" class="post-title"><a style="color: red">{{item.yishengCollectionNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            //系统推荐
            yishengCollectionRecommendList: [],
            dataList: [],
            // 当前详情页表
            detailTable: 'yishengCollection',
        },
        //  清除定时器
        destroyed: function () {
            // 不知道具体作用
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'rate'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.yishengCollectionName;
           // 轮播图片
            vue.swiperList = vue.detail.yishengCollectionPhoto ? vue.detail.yishengCollectionPhoto.split(",") : [];
            var swiperItemHtml = '';
        ...
        http.request(`yishengCollection/list`, 'get', {
            page: 1,
            limit: 5,
            yishengCollectionTypes: vue.detail.yishengCollectionTypes,




        }, function (res) {
            vue.yishengCollectionRecommendList = res.data.list;
        });
