        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.yishengName}}</cite></a>
                </span>

                <!-- 收藏 -->
                <a v-if='storeupFlag' onclick="addYishengCollection()" href="javascript:void(0)">
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.yishengPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.yishengUuidNumber">医生编号：
                            ...
                            <li v-if="detail.yishengShanchang">擅长：
                                {{detail.yishengShanchang}}
                            </li>
                            <li v-if="detail.yishengClicknum">点击次数：
                                {{detail.yishengClicknum}}
                            </li>
                        </ul>
                ...
                <div class="comments-area" v-if="yishengLiuyanDataList.length > 0">
                    <h3 class="title">
                        留言
                    </h3>
                    <ul class="comments-list">
                        <li class="single-comment" v-for="(item,index) in yishengLiuyanDataList" v-bind:key="index">
                            <div class="thumb">
                    ...
                    <form class="comment-form">
                        <div class="row">
                            <div class="col-lg-12 form-group">
                                <textarea name="comment-message" id="yishengLiuyanText" placeholder="留言内容"></textarea>
                            </div>
                            <div class="col-lg-12">
                                <button lay-submit lay-filter="yishengLiuyanText" class="cmn-btn-border">提交</button>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in yishengRecommendList" v-bind:key="index" @click="jump('../yisheng/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.yishengPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.yishengName}}</a>
                                <h6 v-if="item.yishengNewMoney" class="post-title"><a style="color: red">{{item.yishengNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'rate'], function () {
        ...
        http.request(`${vue.detailTable}Liuyan/list`, 'get', {
            ...
            vue.yishengLiuyanDataList = res.data.list;
            // 分页
            laypage.render({
                elem: 'yishengLiuyanPager',
        ...
        // 提交留言
        form.on('submit(yishengLiuyanText)', function (data) {
            data.yonghuId = localStorage.getItem('userid');
            data.tableName = localStorage.getItem('userTable');
            data.yishengLiuyanText =jquery("#yishengLiuyanText").val();
            if(data.yishengLiuyanText == null || data.yishengLiuyanText == ''){
                layer.msg("留言不能为空", {
                    time: 2000,
                    icon: 2
                });
    ...
    // 立即购买
    function addYishengOrder(){
        localStorage.setItem("yishengId",vue.detail.id);
        localStorage.setItem("buyNumber",vue.buyNumber);
        window.location.href='../yishengOrder/add.jsp';
    }
