		...
		<script>
			var vue = new Vue({
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.yishengNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						let data={
                            yishengs:localStorage.getItem('yishengs'),
                            yonghuId: localStorage.getItem('userid'),
						}
                        // 获取商品详情信息
                        layui.http.request(`yishengOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../yishengOrder/list.jsp';
							}
                       		 window.location.href='../yishengOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取医生购买的清单列表
				var yishengs = localStorage.getItem('yishengs');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(yishengs);
